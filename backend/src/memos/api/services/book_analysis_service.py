...
[
  {{
    ...
    "outline": "字符串（必需）- 章节大纲，必须是文本描述格式（纯文本字符串），不是JSON格式。大纲是章节的概要信息，用自然语言描述章节的核心功能、关键情节点、画面感、氛围和结尾钩子等概括性内容。应该是一段连贯的文本描述，清晰简洁地概括章节的整体结构和主要信息。如果找不到则使用空字符串 ''",
    "detailed_outline": "字符串（必需）- 章节细纲，必须是文本描述格式（纯文本字符串），不是JSON格式。细纲是章节的具体情节信息，用自然语言详细描述每个小节的具体内容、情节发展、人物行动、对话要点等细节。应该是一段或多段详细的文本描述，深入描述章节的具体情节展开。如果找不到则使用空字符串 ''"
  }}
]

# 重要说明
- **大纲（outline）**：是章节的概要信息，用自然语言文本描述章节的核心功能、关键情节点、画面感、氛围和结尾钩子等概括性内容，用于快速了解章节的整体结构和主要信息。必须是纯文本格式，不是JSON格式。
- **细纲（detailed_outline）**：是章节的具体情节信息，用自然语言文本详细描述每个小节的具体内容、情节发展、人物行动、对话要点等细节，用于深入了解章节的具体情节展开。必须是纯文本格式，不是JSON格式。
...
- 每个章节必须包含 chapter_number（整数）、title（字符串）、outline（文本字符串）、detailed_outline（文本字符串）
- 用适当的值或空字符串填充所有字段
- JSON前后不要有任何文字
